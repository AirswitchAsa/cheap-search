...
REPO="${CHEAP_SEARCH_INSTALL_REPO:-AirswitchAsa/cheap-search}"
VERSION="${CHEAP_SEARCH_INSTALL_VERSION:-latest}"
INSTALL_DIR="${CHEAP_SEARCH_INSTALL_DIR:-$HOME/.local/bin}"
...
if [ "$os" = "darwin" ] && [ "$arch" = "x64" ]; then
  err "Intel macOS binaries are not published. Install via: uv tool install cheap-search  (or: pip install cheap-search)"
fi

asset="cheap-search-${os}-${arch}"
...
target="$INSTALL_DIR/cheap-search"
mv "$tmp" "$target"
chmod +x "$target"
trap - EXIT
...
info "ok: run 'cheap-search --help' to verify"
